import numpy as np
cimport numpy as np
from cvxopt import matrix, solvers as cvxopt_solvers
...
def solve_proxqp(double cd, double[:] avec1_mv, double[:] sigvec1_mv):
    try:
        import proxsuite
    except ImportError:
        raise ImportError(
            "proxsuite is required for solve_proxqp but not installed.\n"
            "Install via conda: conda install -c conda-forge proxsuite"
        )
    avec1 = np.asarray(avec1_mv, dtype=np.float64)
    sigvec1 = np.asarray(sigvec1_mv, dtype=np.float64)
    n = avec1.shape[0]

    H = np.eye(n) + cd * np.ones((n, n))
    g = -avec1

    lb = -100.0 * (sigvec1 == -1.0)
    ub = 100.0 * (sigvec1 == 1.0)

    qp = proxsuite.proxqp.dense.QP(n, 0, 0, True)
    qp.init(H, g, None, None, None, None, None, lb, ub)
    qp.solve()
    return np.asarray(qp.results.x).flatten()

def solve_cvxopt(double cd, double[:] avec1_mv, double[:] sigvec1_mv):
    avec1 = np.asarray(avec1_mv, dtype=np.float64)
    sigvec1 = np.asarray(sigvec1_mv, dtype=np.float64)
    n = avec1.shape[0]

    P = matrix(np.eye(n) + cd * np.ones((n, n)))
    q = matrix(-avec1)

    lb = -100.0 * (sigvec1 == -1.0)
    ub = 100.0 * (sigvec1 == 1.0)
    G_np = np.vstack([-np.eye(n), np.eye(n)])
    h_np = np.concatenate([-lb, ub])
    G = matrix(G_np)
    h = matrix(h_np)
